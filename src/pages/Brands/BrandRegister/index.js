...
const useBrandRegister = (id) => {
  ...
  const handleSubmit = (e) => {
    ...
    if(id) {
      ...
      .then(json => {
        if(json.status)
          success('Marca registrada con exito!')
            .then(() => navigate(RESOURCE))
      })
    }
  }

  return {inputLabel, labelWidth, loading, data, name, status, handleSubmit}
}

export const BrandRegister = ({ id = undefined, title  }) => {
  const classes = useStyles();
  const {inputLabel, labelWidth, loading, data, name, status, handleSubmit} = useBrandRegister(id)
    ...
    <div className={classes.root}>
      ...
      <form onSubmit={handleSubmit}>
        <Grid container spacing={3}>
          ...
          <MyTitleBar back={RESOURCE} title={`${title} Marca`} loading={loading} />
