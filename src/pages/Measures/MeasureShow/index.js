...
export const MeasureShow = () => {
          ...
          return (
            <Fragment>
              ...
              <a onClick={() => {
                const index = tableMeta.rowIndex
                const id = dataTable[index].id
                confirm({ title: 'Mensaje', message: '¿Estas seguro de querer eliminar esta media?' })
                .then(result => {
                  if(result) {
  ...
  return (
    <DataTable title={'MEDIDAS'} columns={columns} dataSource={dataTable} url={`${RESOURCE}/register`} />
  )
}