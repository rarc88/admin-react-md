...
export const ItemShow = () => {
          ...
          return (
            <Fragment>
              ...
              <a onClick={() => {
                const index = tableMeta.rowIndex
                const id = dataTable[index].id
                confirm({ title: 'Mensaje', message: '¿Estas seguro de querer eliminar este producto?' })
                .then(result => {
                  if(result) {
  ...
  return (
    <DataTable title={'PRODUCTOS'} columns={columns} dataSource={dataTable} url={`${RESOURCE}/register`} />
  )
}