...
const useProviderRegister = (id) => {
  ...
  const identityDocument = useInputValue('')
    ...
    if(id) {
      fetchData({ resource: `${RESOURCE}/${id}/detail`, method: 'GET' })
        .then(json => {
          ...
          identityDocument.setValue(provider.identity_document)
          name.setValue(provider.name)
          phone.setValue(provider.phone)
          status.setValue(provider.status)
        })
    }
  }, []);
  ...
  const handleSubmit = (e) => {
    ...
    if(id) {
      ...
      .then(json => {
        if(json.status)
          success('Proveedor registrado con exito!')
            .then(() => navigate(RESOURCE))
      })
    }
  }

  return {inputLabel, labelWidth, loading, data, identityDocument, name, phone, status, handleSubmit}
}

export const ProviderRegister = ({ id = undefined, title  }) => {
  const classes = useStyles();
  const {inputLabel, labelWidth, loading, data, identityDocument, name, phone, status, handleSubmit} = useProviderRegister(id)
    ...
    <div className={classes.root}>
      ...
      <form onSubmit={handleSubmit}>
        <Grid container spacing={3}>
          ...
          <MyTitleBar back={RESOURCE} title={`${title} Proveedor`} loading={loading} />
