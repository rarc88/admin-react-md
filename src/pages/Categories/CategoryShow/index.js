...
export const CategoryShow = () => {
          ...
          return (
            <Fragment>
              ...
              <a onClick={() => {
                const index = tableMeta.rowIndex
                const id = dataTable[index].id
                confirm({ title: 'Mensaje', message: '¿Estas seguro de querer eliminar esta categoria?' })
                .then(result => {
                  if(result) {
  ...
  return (
    <DataTable title={'CATEGORIAS'} columns={columns} dataSource={dataTable} url={`${RESOURCE}/register`} />
  )
}